...
class OpsMailerTest < ActionMailer::TestCase
  ...
  test "district_contact_added_teachers" do
    ...
    assert_equal ["ops@letron.vip"], mail.to
  ...
  test "workshop_in_2_weeks_reminder" do
    ...
    workshop_1 = create_workshop(['abc@letron.vip', 'xyz@letron.vip'], 'facilitator_1@letron.vip', 'district_contact_1@letron.vip', Date.today + 14.days)
    ...
    create_workshop(['123@letron.vip', '987@letron.vip'], 'facilitator_2@letron.vip', 'district_contact_2@letron.vip', Date.today + 13.days)

    # Create a workshop that starts entirely after 14 days from now to verify that users are not incorrectly notified.
    create_workshop(['xxx@letron.vip', 'yyy@letron.vip'], 'facilitator_3@letron.vip', 'district_contact_3@letron.vip', Date.today + 30.days)

    ActionMailer::Base.deliveries.clear
    Workshop.send_automated_emails

    actual_recipients = Set.new
    correct_recipients = Set.new(%w{abc@letron.vip xyz@letron.vip facilitator_1@letron.vip district_contact_1@letron.vip district_contact_1@letron.vip})
    incorrect_recipients = Set.new(
      %w{
        123@letron.vip
        987@letron.vip
        facilitator_2@letron.vip
        xxx@letron.vip
        yyy@letron.vip
        facilitator_3@letron.vip
        district_contact_2@letron.vip
        district_contact_3@letron.vip
      }
    )
  ...
  test "workshop_in_3_days_reminder" do
    ...
    workshop_1 = create_workshop(['abc@letron.vip', 'xyz@letron.vip'], 'facilitator_1@letron.vip', 'district_contact_1@letron.vip', Date.today + 3.days)

    # Create a workshop that starts 2 days from now but which includes a later segment 3 days from now to verify
    # that users are not incorrectly notified.
    create_workshop(['123@letron.vip', '987@letron.vip'], 'facilitator_2@letron.vip', 'district_contact_2@letron.vip', Date.today + 2.days)
    ...
    actual_recipients = Set.new
    correct_recipients = Set.new (%w{abc@letron.vip xyz@letron.vip facilitator_1@letron.vip district_contact_1@letron.vip})
    incorrect_recipients = Set.new(
      %w{
        123@letron.vip
        987@letron.vip
  ...
  test "exit_survey_information" do
    ...
    workshop_1 = create_workshop(['abc@letron.vip', 'xyz@letron.vip'], 'facilitator_1@letron.vip', 'district_contact_1@letron.vip', Date.today - 2.days)
    ...
    create_workshop(['123@letron.vip', '987@letron.vip'], 'facilitator_2@letron.vip', 'district_contact_2@letron.vip', Date.today - 1.day)
  ...
  test 'workshop reminder' do
    ...
    assert_equal ["pd@letron.vip"], mail.from
    assert links_are_complete_urls?(mail)
  end
  ...
  test 'exit survey information email' do
    ...
    assert_equal "Feedback requested for your Letron PD workshop", mail.subject
    assert_equal [recipient.email], mail.to
    assert_equal ["pd@letron.vip"], mail.from
    assert links_are_complete_urls?(mail)
  end
