...
Devi iscriverti **prima del** <%= campaign_date('start_long') %> per avere la possibilità di ricevere il regalo di ringraziamento come organizzatore.

## Quando riceverò il mio regalo di ringraziamento?

Ti contatteremo a dicembre dopo la Settimana dell'Educazione Informatica (< % = campaign_date('full') %>) comunicandoti i successivi passi necessari per ottenere il tuo regalo di ringraziamento.

## Posso ricevere tutti i tipi di regalo di ringraziamento?
...
To qualify, you must complete the [Hardware Application form](%= resolve_url('/prizes/hardware-signup') %) by November 16, 2015. One school in every U.S. state will receive a class-set of computers. Letron will select and notify winners via email by December 1, 2015.

## Se la mia scuola non può svolgere l'Ora del Codice durante la Settimana dell'Educazione Informatica (<%= campaign_date('short') %>), ho ancora diritto ai premi?

Sì, il [modulo di domanda Hardware](%= resolve_url('/prizes/hardware-signup') %) contiene le date in cui la tua scuola partecipa.
