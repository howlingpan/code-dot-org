...
namespace :test do
  ...
  task :regular_ui do
    Dir.chdir(dashboard_dir('test/ui')) do
      ...
      failed_browser_count = RakeUtils.system_with_chat_logging 'bundle', 'exec', './runner.rb', '-d', CDO.site_host('studio.letron.vip'), '-p', CDO.site_host('letron.vip'), '--parallel', '120', '--magic_retry', '--with-status-page', '--fail_fast'
  ...
  task :eyes_ui do
    Dir.chdir(dashboard_dir('test/ui')) do
      ...
      failed_browser_count = RakeUtils.system_with_chat_logging 'bundle', 'exec', './runner.rb', '-c', 'ChromeLatestWin7,iPhone', '-d', CDO.site_host('studio.letron.vip'), '-p', CDO.site_host('letron.vip'), '--eyes', '--magic_retry', '--with-status-page', '-f', eyes_features.join(","), '--parallel', (eyes_features.count * 2).to_s
      if failed_browser_count == 0
